...
export default class Block {
  constructor(counter, audio) {
    ...
    geometryFloor.applyMatrix4(
      new Matrix4().makeTranslation(0, -thickness / 2, 0)
    );
    geometryFloor.merge(
      geometryFloor,
      new Matrix4().makeTranslation(0, height + thickness, 0)
    );
    geometryFloor.merge(
      geometryWall,
      new Matrix4().makeTranslation(width / 2 + thickness / 2, height / 2, 0)
    );
    geometryFloor.merge(
      geometryWall,
      new Matrix4().makeTranslation(-width / 2 - thickness / 2, height / 2, 0)
    ...
    const special =
      Config.rectLight.crash.enabled &&
      counter === Config.rectLight.crash.number - 1;
