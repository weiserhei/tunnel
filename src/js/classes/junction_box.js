...
export default class JunctionBox {
  constructor(material, sound) {
    ...
    const mesh = new Mesh(
      new BoxBufferGeometry(0.1, 0.6, 0.45),
      material.clone()
    );
    ...
    const button = new Mesh(
      new BoxBufferGeometry(0.2, 0.1, 0.3),
      buttonMaterial
    );
    ...
    var circle = new Mesh(
      circle_geometry,
      new MeshPhongMaterial({ emissive: normal_color })
    );
    ...
    var cable_material = new MeshPhongMaterial({
      color: 0x999999,
      emissive: 0x999955,
    });
